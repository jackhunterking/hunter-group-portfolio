...
Read `CLAUDE.md` and `hunter-merged-site-build-spec.md` first, then implement **Phase 1 only: the Finansman core.** Do not start later phases.

Scope for this phase:

1. Create a new route `app/finansman/page.tsx` — a Turkish-first landing page with:
...
2. Create `app/finansman/[intent]/page.tsx` handling these intents:
   `ev-almak`, `yenileme`, `tadilat`, `borc-toparlama`, `ev-degeri`, `heloc`.

3. Add a `FinansmanTeaser` section component and insert it into `app/page.tsx` after `ServicesSection`.

4. Add a **Finansman** item to the main nav in `components/Nav.tsx`, using the existing `t.*` i18n pattern.
